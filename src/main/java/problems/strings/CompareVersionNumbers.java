...
This means that revisions 1 and 001 are considered equal. If a version number does not specify a
revision at an index, then treat the revision as 0. For example, version 1.0 is less than version 1.1
...
public class CompareVersionNumbers {
    public static void main(String[] args) {
        System.out.println(new CompareVersionNumbers().compareVersion("1.1", "1.1.1"));
    ...
    public int compareVersion(String version1, String version2) {
        ...
        while(i<ver1Arr.length && j<ver2Arr.length){
            num1 = Integer.parseInt(ver1Arr[i++]);
            num2 = Integer.parseInt(ver2Arr[j++]);
            if(num1 < num2)
                return -1;
            else if (num1 > num2)
                return 1;
        }
        // for case 1.0.1 and 1.0
        if(i != ver1Arr.length)
            while (i < ver1Arr.length)
                if(Integer.parseInt(ver1Arr[i++]) > 0)
                    return 1;
        if(j != ver2Arr.length)
            while (j < ver2Arr.length)
                if(Integer.parseInt(ver2Arr[j++]) > 0)
                    return -1;
        return 0;
    }
}
